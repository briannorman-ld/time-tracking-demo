...
export function CustomerModal({
  ...
  const handleCreate = async () => {
    ...
    setSaving(true)
    try {
      const created = await createCustomer(user.id, editName.trim())
      onCreated?.(created)
      onClose()
    } finally {
      setSaving(false)
  ...
  const handleSaveName = async () => {
    if (!user || !customer || editName.trim() === customer.name) return
    setSaving(true)
    try {
    ...
    <div className="customer-modal-backdrop" onClick={onClose}>
      ...
      >
        <div className="customer-modal-header">
          <h2 id="customer-modal-title">{isCreate ? 'New customer' : 'Customer'}</h2>
          <button
            type="button"
          ...
          <section className="customer-modal-section">
            ...
            <div className="customer-modal-name-row">
              <input
                ...
                placeholder={isCreate ? 'Customer name' : undefined}
              />
              {isCreate ? (
                <button
                  type="button"
